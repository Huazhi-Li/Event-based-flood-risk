...
# i=138
# Event folder
event_folder = os.path.join(path,'Eventset/'+basin+'/rp_event_set_cluster{:d}.pkl'.format(cluster0))
...
# Other input files
if basin=='NEA' and cluster0==28: cluster=27
else: cluster=cluster0
...
if inun_event.max()>0: 
    ...
    # Save to nc 
    fn = os.path.join(path, 'Hazard/Spatial_inun/'+basin+'/cluster{:d}'.format(cluster0))
